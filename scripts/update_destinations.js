...
const DATA_PATH = path.join(__dirname, '..', 'server', 'data', 'india_destinations.json');
...
function getCategory(name = '', type = '') {
  ...
  if (t.includes('temple') || t.includes('church') || t.includes('mosque') || t.includes('mandir') || t.includes('dargah') || t.includes('monastery') || n.includes('temple') || n.includes('mandir') || n.includes('dargah') || n.includes('monastery')) {
    return 'temple';
  }
  if (t.includes('fort') || n.includes('fort') || n.includes('kila')) {
    return 'fort';
  }
  if (t.includes('palace') || t.includes('mahal') || n.includes('palace') || n.includes('mahal')) {
    return 'palace';
  }
  if (t.includes('beach') || n.includes('beach') || t.includes('sea') || n.includes('sea')) {
    return 'beach';
  }
  if (t.includes('lake') || t.includes('waterfall') || t.includes('valley') || t.includes('park') || t.includes('hill') || t.includes('trek') || t.includes('mountain') || n.includes('lake') || n.includes('waterfall') || n.includes('valley') || n.includes('park') || n.includes('hill')) {
    return 'nature';
  }
  if (t.includes('museum') || t.includes('gallery') || n.includes('museum') || n.includes('gallery')) {
    return 'museum';
  }
  if (t.includes('market') || t.includes('mall') || t.includes('bazaar') || n.includes('market') || n.includes('mall') || n.includes('bazaar') || n.includes('chowk')) {
    return 'market';
  }
  return 'default';
}
...
async function main() {
  ...
  for (let i = 0; i < destinations.length; i++) {
    ...
    const needsCoords = !dest.coordinates || dest.coordinates.lat === null || dest.coordinates.lon === null;
    const firstImage = dest.images?.[0] || '';
    const needsImage = !firstImage || firstImage.includes('loremflickr.com') || firstImage.includes('placeholder');
    ...
    if (needsCoords || needsImage) {
      ...
      try {
        ...
        if (searchResults && searchResults.length > 0) {
          ...
          if (dest.city && (bestMatchTitle.toLowerCase() === 'sunset point' || bestMatchTitle.toLowerCase() === 'sunrise point')) {
            const refinedResults = await searchWiki(`${dest.name} ${dest.city}`);
        ...
        if (bestMatchTitle) {
          ...
          if (details.coordinates) {
            ...
            console.log(`  -> Coords found on Wikipedia: [${resolvedCoords.lat}, ${resolvedCoords.lon}]`);
          }
          
          if (details.original) {
            resolvedImage = details.original.source;
            console.log(`  -> Real photograph found: ${resolvedImage.substring(0, 70)}...`);
          } else if (details.thumbnail) {
            resolvedImage = details.thumbnail.source;
            console.log(`  -> Real photo (thumbnail) found: ${resolvedImage.substring(0, 70)}...`);
          }
      ...
      if (needsCoords && !resolvedCoords) {
        ...
        resolvedCoords = await getCoordinatesNominatim(dest.name, dest.city, dest.state);
        if (resolvedCoords) {
          console.log(`  -> Coords resolved via Nominatim: [${resolvedCoords.lat}, ${resolvedCoords.lon}]`);
        } else {
          ...
          else if (dest.city === 'Delhi') center = { lat: 28.6139, lon: 77.2090 };
          else if (dest.city === 'Mumbai') center = { lat: 19.0760, lon: 72.8777 };
          else if (dest.city === 'Bangalore') center = { lat: 12.9716, lon: 77.5946 };
          else if (dest.city === 'Kolkata') center = { lat: 22.5726, lon: 88.3639 };
          else if (dest.city === 'Goa') center = { lat: 15.2993, lon: 74.1240 };
          else if (dest.city === 'Hyderabad') center = { lat: 17.3850, lon: 78.4867 };
          
          resolvedCoords = {
            lat: parseFloat((center.lat + (Math.random() - 0.5) * 0.01).toFixed(6)),
            lon: parseFloat((center.lon + (Math.random() - 0.5) * 0.01).toFixed(6))
          };
          console.log(`  -> Fallback city/state coords applied: [${resolvedCoords.lat}, ${resolvedCoords.lon}]`);
        }
      ...
      if (needsImage) {
        ...
        dest.images = [
          ...
          unsplashCategoryImages.default[updatedCount % unsplashCategoryImages.default.length]
        ];
        imagesCleaned++;
        changed = true;
      }
      ...
      const needsSecondaryImageClean = dest.images.some(img => img.includes('loremflickr.com'));
      if (needsSecondaryImageClean) {
        ...
        dest.images = [
          ...
          secondaryImages[1] || secondaryImages[0],
          secondaryImages[2] || secondaryImages[0],
          unsplashCategoryImages.default[i % unsplashCategoryImages.default.length]
        ];
        imagesCleaned++;
        updatedCount++;
      }
    ...
    if (i > 0 && i % 10 === 0) {
      ...
      console.log(`[Incremental Save] Successfully saved progress at item ${i}...`);
    }
  }
